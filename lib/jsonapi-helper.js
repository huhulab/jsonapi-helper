(function () {
  ...
  JSONAPIHelper.prototype.expand = function () {
    ...
    // Keep `meta` info since it's required for client pagination.
    ...
    if (this._type === 'collection') {
      this._expandedObject.data = this._expandedObject[this._mainKey];
      delete this._expandedObject[this._mainKey];
    }
    else {
      this._expandedObject = mainExpandedObject;
    }

    for (key in this._jsonapiObject) {
      delete this._jsonapiObject[key];
    }

    for (key in this._expandedObject) {
      this._jsonapiObject[key] = this._expandedObject[key];
    }
